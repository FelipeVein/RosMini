...
def communicate_rosminicore(sock, topic):
    ...
    len_topic_bytes = bytes(str(len(topic)))
...
class Publisher:
    ...
    def send(self, data):
        len_data = bytes(len(data)).ljust(16, b' ')
        self.sock.sendto(len_data, (MCAST_ADDR, self.MCAST_PORT))
        self.sock.sendto(bytes(data), (MCAST_ADDR, self.MCAST_PORT))
...
class Subscriber:
    ...
    def receive(self):
        ...
        data, addr = self.sock.recvfrom(len_data)
        return data.decode()
...
if __name__ == '__main__':
    ...
    if args.publish:
        ...
        while True:
            ...
            pub.send(b'Hi' + b' ' + bytes(str(time.time())))
            time.sleep(0.1)
